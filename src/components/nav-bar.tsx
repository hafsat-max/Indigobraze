      ...
      <div className="flex items-center gap-8">
        ...
        <ul className="flex items-center gap-32 max-[600px]:hidden">
          {list.map(({ text, id, link }) => (
            <li
              key={id}
              className="text-18 list-none text-grey hover:text-primary active:text-primary cursor-pointer"
            >
              <a href={`${link}`}>{text}</a>
            </li>
          ))}
          ...
          <Menu.Item color="red">
            <Flex direction="column" gap={12}>
              {list.map(({ text, id, link }) => (
                <li
                  key={id}
                  className="text-18 list-none text-grey cursor-pointer  hover:bg-secondary"
                >
                  <a href={link}>{text}</a>
                </li>
              ))}
